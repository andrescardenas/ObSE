...
if ( strcmp('contrato',$mensaje->accion) == 0) {
	...
	/*if( count($existe_identificacion[existe_identificacion]) != 0 ){
		...
		$SQLInsertUsuario = "INSERT INTO usuarios (id, usuario, primer_nombre, segundo_nombre, primer_apellido, segundo_apellido, clave, recordatorio, bl_acepto, valoracion) 
			VALUES (NULL, '".$mensaje->email."', '".$mensaje->primer_nombre."', '".$mensaje->segundo_nombre."', '".$mensaje->primer_apellido."', '".$mensaje->segundo_apellido."', '".stringToPass($mensaje->clave)."', '".$mensaje->recordatorio."', '1', '0');";
	...
	//}
}else if ( strcmp('registro', $mensaje->accion) == 0 ){
	$SQLInsertUsuario = "INSERT INTO usuarios (id, usuario, primer_nombre, segundo_nombre, primer_apellido, segundo_apellido, clave, recordatorio, bl_acepto, valoracion) 
			VALUES (NULL, '".$mensaje->email."', '".$mensaje->primer_nombre."', '".$mensaje->segundo_nombre."', '".$mensaje->primer_apellido."', '".$mensaje->segundo_apellido."', '".stringToPass($mensaje->clave)."', '".$mensaje->recordatorio."', '1', '0');";
	$nuevo_id_usuario = insertarFila($SQLInsertUsuario);
	$datos = array('status'=> 'OK', 'mensaje'=>'Usuario Registrado Correctamente ['.$nuevo_id_usuario.']', 'user' => array( 'id' => $nuevo_id_usuario, 'nombre' => $mensaje->primer_nombre.' '.$mensaje->primer_apellido, 'usuario' => $mensaje->usuario) );
}
