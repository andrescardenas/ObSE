...
foreach ($datos[resultados] as $key => $value) {
	...
	//si el usuario tiene @
	if ( strpos($datos[resultados][$key][usuario], "@") !== FALSE ){
		$explode_nombreCorreo = explode("@", $datos[resultados][$key][usuario]);
		$dominio_temporal = $explode_nombreCorreo[0];
		$explode_dominioCorreo = explode(".", $explode_nombreCorreo[1]);
		$dominio_temporal .= "_".$explode_dominioCorreo[0]."_".$explode_dominioCorreo[1];
		$datos[resultados][$key][usuario] = $dominio_temporal;
	}
}
