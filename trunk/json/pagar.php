...
$SQLMaximoNivel = "	SELECT COUNT(nivel) as niveles
					FROM r2_planes_niveles
					GROUP BY hd_planes
...
for ($i=0; $i < $max; $i++) { 
	...
	if ( intval(isset($niveles_padre[niveles_padre][$i])) == 1 ){
		...
		if ( intval($niveles_padre[niveles_padre][$i][bl_activo]) == 1){
			...
			VALUES (NULL, ".$niveles_padre[niveles_padre][$i][usuario_plan].", '1', '".date("Y-m-d H:i:s")."', 1, ".$monto_pago.", 1, '1', 1)"; echo "<br />............{{".$SQLPagar."}}";
